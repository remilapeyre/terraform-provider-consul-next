// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

package datasource

import (
	"context"

	"github.com/hashicorp/consul/api"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/remilapeyre/terraform-provider-consul-next/internal/models"
	"github.com/remilapeyre/terraform-provider-consul-next/internal/structs"
)

func NewPeerings() datasource.DataSource {
	return NewDataSource(
		"peerings",
		peeringsSchema(),
		&Peerings{},
	)
}

type Peerings struct{}

func (d *Peerings) Read(ctx context.Context, client *api.Client, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var peering *structs.Peerings
	resp.Diagnostics.Append(models.DecodePeerings(ctx, req.Config, &peering)...)
	if resp.Diagnostics.HasError() {
		return
	}

	peers, _, err := client.Peerings().List(ctx, nil)
	if err != nil {
		resp.Diagnostics.AddError("failed to list peerings", err.Error())
		return
	}

	peering.ID = "peerings"
	peering.Peerings = peers
	resp.Diagnostics.Append(models.Set(ctx, &resp.State, peering)...)
}
