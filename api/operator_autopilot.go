...
type AutopilotConfiguration struct {
	...
	CleanupDeadServers bool `terraform:"cleanup_dead_servers,optional,computed"`

	// LastContactThreshold is the limit on the amount of time a server can go
	// without leader contact before being considered unhealthy.
	LastContactThreshold *ReadableDuration // TODO: `terraform:"last_contact_threshold,optional,computed"`

	// MaxTrailingLogs is the amount of entries in the Raft Log that a server can
	// be behind before being considered unhealthy.
	MaxTrailingLogs uint64 `terraform:"max_trailing_logs,optional,computed"`

	// MinQuorum sets the minimum number of servers allowed in a cluster before
	// autopilot can prune dead servers.
	MinQuorum uint `terraform:"min_quorum,optional,computed"`

	// ServerStabilizationTime is the minimum amount of time a server must be
	// in a stable, healthy state before it can be added to the cluster. Only
	// applicable with Raft protocol version 3 or higher.
	ServerStabilizationTime *ReadableDuration // TODO `terraform:"server_stabilization_time,optional,computed"`

	// (Enterprise-only) RedundancyZoneTag is the node tag to use for separating
	// servers into zones for redundancy. If left blank, this feature will be disabled.
	RedundancyZoneTag string `terraform:"redundancy_zone_tag,optional,computed"`

	// (Enterprise-only) DisableUpgradeMigration will disable Autopilot's upgrade migration
	// strategy of waiting until enough newer-versioned servers have been added to the
	// cluster before promoting them to voters.
	DisableUpgradeMigration bool `terraform:"disable_upgrade_migration,optional,computed"`

	// (Enterprise-only) UpgradeVersionTag is the node tag to use for version info when
	// performing upgrade migrations. If left blank, the Consul version will be used.
	UpgradeVersionTag string `terraform:"upgrade_version_tag,optional,computed"`
...
type ServerHealth struct {
	...
	SerfStatus string `terraform:"serf_status"`

	// Version is the Consul version of the server.
	Version string `terraform:"version"`

	// Leader is whether this server is currently the leader.
	Leader bool `terraform:"leader"`

	// LastContact is the time since this node's last contact with the leader.
	LastContact *ReadableDuration // TODO: add support this type

	// LastTerm is the highest leader term this server has a record of in its Raft log.
	LastTerm uint64 `terraform:"last_term"`

	// LastIndex is the last log index this server has a record of in its Raft log.
	LastIndex uint64 `terraform:"last_index"`

	// Healthy is whether or not the server is healthy according to the current
	// Autopilot config.
	Healthy bool `terraform:"healthy"`

	// Voter is whether this is a voting server.
	Voter bool `terraform:"voter"`

	// StableSince is the last time this server's Healthy value changed.
	StableSince time.Time `terraform:"stable_since"`
}
...
type OperatorHealthReply struct {
	...
	// an outage occurring.
	FailureTolerance int `terraform:"failure_tolerance"`

	// Servers holds the health of each server.
	Servers []ServerHealth `terraform:"servers"`
}
