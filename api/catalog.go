...
type Weights struct {
	Passing int `terraform:"passing"`
	Warning int `terraform:"warning"`
}

type Node struct {
	ID              string            `terraform:"id"`
	Node            string            `terraform:"node"`
	Address         string            `terraform:"address"`
	Datacenter      string            `terraform:"datacenter"`
	TaggedAddresses map[string]string `terraform:"tagged_addresses"`
	Meta            map[string]string `terraform:"meta"`
	CreateIndex     uint64
	ModifyIndex     uint64
	Partition       string    `json:",omitempty" terraform:"partition"`
	PeerName        string    `json:",omitempty" terraform:"peer_name"`
	Locality        *Locality `json:",omitempty" terraform:"locality"`
}
...
type CatalogService struct {
	...
	Datacenter               string                          `terraform:"datacenter"`
	TaggedAddresses          map[string]string               `terraform:"tagged_addresses"`
	NodeMeta                 map[string]string               `terraform:"node_meta"`
	ServiceID                string                          `terraform:"service_id"`
	ServiceName              string                          `terraform:"service_name"`
	ServiceAddress           string                          `terraform:"service_address"`
	ServiceTaggedAddresses   map[string]ServiceAddress       `terraform:"service_tagged_addresses"`
	ServiceTags              []string                        `terraform:"service_tags"`
	ServiceMeta              map[string]string               `terraform:"service_meta"`
	ServicePort              int                             `terraform:"service_port"`
	ServiceWeights           Weights                         `terraform:"service_weights"`
	ServiceEnableTagOverride bool                            `terraform:"service_enable_tag_override"`
	ServiceProxy             *AgentServiceConnectProxyConfig `terraform:"service_proxy"`
	ServiceLocality          *Locality                       `json:",omitempty" terraform:"service_locality"`
	CreateIndex              uint64                          `terraform:"create_index"`
	Checks                   HealthChecks                    `terraform:"checks"`
	ModifyIndex              uint64                          `terraform:"modify_index"`
	Namespace                string                          `json:",omitempty" terraform:"namespace"`
	Partition                string                          `json:",omitempty" terraform:"partition"`
}

type CatalogNode struct {
	Node     *Node                    `terraform:"-,promoted"`
	Services map[string]*AgentService `terraform:"services"`
}
