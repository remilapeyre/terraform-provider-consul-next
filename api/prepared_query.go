...
type QueryFailoverOptions struct {
	...
	// network coordinates.
	NearestN int `terraform:"nearest_n"`
	...
	Targets []QueryFailoverTarget `terraform:"targets"`
}
...
type ServiceQuery struct {
	...
	SamenessGroup string `json:",omitempty" terraform:"sameness_group"`
	...
	Near string `terraform:"near"`

	// Failover controls what we do if there are no healthy nodes in the
	// local datacenter.
	Failover QueryFailoverOptions `json:",omitempty" terraform:"failover"`

	// IgnoreCheckIDs is an optional list of health check IDs to ignore when
	// considering which nodes are healthy. It is useful as an emergency measure
	// to temporarily override some health check that is producing false negatives
	// for example.
	IgnoreCheckIDs []string `terraform:"ignore_check_ids"`

	// If OnlyPassing is true then we will only include nodes with passing
	// health checks (critical AND warning checks will cause a node to be
	// discarded)
	OnlyPassing bool `terraform:"only_passing"`
	...
	Connect bool `terraform:"connect"`
}
...
type QueryTemplate struct {
	...
	Regexp string `terraform:"regexp"`
	...
	RemoveEmptyTags bool `terraform:"remove_empty_tags"`
}
...
type PreparedQueryDefinition struct {
	...
	Session string `terraform:"session"`

	// Token is the ACL token used when the query was created, and it is
	// used when a query is subsequently executed. This token, or a token
	// with management privileges, must be used to change the query later.
	Token string `terraform:"token"`

	// Service defines a service query (leaving things open for other types
	// later).
	Service ServiceQuery `terraform:"service"`

	// DNS has options that control how the results of this query are
	// served over DNS.
	DNS QueryDNSOptions `terraform:"dns,optional,computed"`
	...
	Template QueryTemplate `terraform:"template,optional,computed"`
}
