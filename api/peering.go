...
type Locality struct {
	...
	Region string `terraform:"region"`

	// Zone is the zone the entity is running in.
	Zone string `terraform:"zone"`
}
...
type Peering struct {
	...
	DeletedAt *time.Time `json:",omitempty" alias:"deleted_at" terraform:"deleted_at"`
	...
	State PeeringState `terraform:"state"`
	...
	PeerID string `json:",omitempty" terraform:"peer_id"`
	// PeerCAPems contains all the CA certificates for the remote peer.
	PeerCAPems []string `json:",omitempty" terraform:"peer_ca_pems"`
	// PeerServerName is the name of the remote server as it relates to TLS.
	PeerServerName string `json:",omitempty" terraform:"peer_server_name"`
	// PeerServerAddresses contains all the connection addresses for the remote peer.
	PeerServerAddresses []string `json:",omitempty" terraform:"peer_server_addresses"`
	// StreamStatus contains information computed on read based on the state of the stream.
	StreamStatus PeeringStreamStatus `terraform:"stream_status"`
...
type PeeringGenerateTokenRequest struct {
	...
	PeerName string `terraform:"peer_name"`
	...
	ServerExternalAddresses []string `json:",omitempty" terraform:"server_external_addresses"`
}
...
type PeeringGenerateTokenResponse struct {
	...
	PeeringToken string `terraform:"peering_token,computed"`
}
...
type PeeringEstablishRequest struct {
	...
	PeeringToken string `json:",omitempty" terraform:"peering_token"`
	// Partition to be peered.
	Partition string `json:",omitempty" terraform:"partition"`
	// Meta is a mapping of some string value to any other string value
	Meta map[string]string `json:",omitempty" terraform:"meta"`
