...
type MeshGatewayConfig struct {
	...
	Mode MeshGatewayMode `json:",omitempty" terraform:"mode"`
}
...
type TransparentProxyConfig struct {
	...
	OutboundListenerPort int `json:",omitempty" alias:"outbound_listener_port" terraform:"outbound_listener_port"`
	...
	DialedDirectly bool `json:",omitempty" alias:"dialed_directly" terraform:"dialed_directly"`
}
...
type ExposeConfig struct {
	...
	Checks bool `json:",omitempty" terraform:"checks"`

	// Paths is the list of paths exposed through the proxy.
	Paths []ExposePath `json:",omitempty" terraform:"paths"`
}

// EnvoyExtension has configuration for an extension that patches Envoy resources.
type EnvoyExtension struct {
	Name          string                 `terraform:"name"`
	Required      bool                   `terraform:"required"`
	Arguments     map[string]interface{} `bexpr:"-" terraform:"arguments"`
	ConsulVersion string                 `terraform:"consul_version"`
	EnvoyVersion  string                 `terraform:"envoy_version"`
}

type ExposePath struct {
	// ListenerPort defines the port of the proxy's listener for exposed paths.
	ListenerPort int `json:",omitempty" alias:"listener_port" terraform:"listener_port"`

	// Path is the path to expose through the proxy, ie. "/metrics."
	Path string `json:",omitempty" terraform:"path"`

	// LocalPathPort is the port that the service is listening on for the given path.
	LocalPathPort int `json:",omitempty" alias:"local_path_port" terraform:"local_path_port"`
	...
	ParsedFromCheck bool `terraform:"parsed_from_check"`
}
...
type AccessLogsConfig struct {
	...
	Enabled bool `json:",omitempty" alias:"enabled" terraform:"enabled"`
	...
	DisableListenerLogs bool `json:",omitempty" alias:"disable_listener_logs" terraform:"disable_listener_logs"`

	// Type selects the output for logs: "file", "stderr". "stdout"
	Type LogSinkType `json:",omitempty" alias:"type" terraform:"type"`

	// Path is the output file to write logs
	Path string `json:",omitempty" alias:"path" terraform:"path"`

	// The presence of one format string or the other implies the access log string encoding.
	// Defining Both is invalid.
	JSONFormat string `json:",omitempty" alias:"json_format" terraform:"json_format"`
	TextFormat string `json:",omitempty" alias:"text_format" terraform:"text_format"`
}
