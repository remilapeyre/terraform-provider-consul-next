...
type Namespace struct {
	...
	Name string `json:"Name" terraform:"name,required"`

	// Description is where the user puts any information they want
	// about the namespace. It is not used internally.
	Description string `json:"Description,omitempty" terraform:"description"`
	...
	ACLs *NamespaceACLConfig `json:"ACLs,omitempty" terraform:"ac_ls"`

	// Meta is a map that can be used to add kv metadata to the namespace definition
	Meta map[string]string `json:"Meta,omitempty" terraform:"meta"`

	// DeletedAt is the time when the Namespace was marked for deletion
	// This is nullable so that we can omit if empty when encoding in JSON
	DeletedAt *time.Time `json:"DeletedAt,omitempty" alias:"deleted_at" terraform:"deleted_at"`

	// Partition which contains the Namespace.
	Partition string `json:"Partition,omitempty" terraform:"partition"`
...
type NamespaceACLConfig struct {
	...
	PolicyDefaults []ACLLink `json:"PolicyDefaults" alias:"policy_defaults" terraform:"policy_defaults"`
	// RoleDefaults is the list of roles that should be used for the parent authorizer
	// of all tokens in the associated namespace.
	RoleDefaults []ACLLink `json:"RoleDefaults" alias:"role_defaults" terraform:"role_defaults"`
}
